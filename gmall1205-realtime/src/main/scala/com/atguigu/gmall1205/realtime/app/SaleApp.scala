...
import com.atguigu.gmall1205.realtime.util.{MyEsUtil, MyKafkaUtil, RedisUtil}
...
object SaleApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val orderInfoDstream: DStream[OrderInfo] = inputOrderDstream.map { record =>
      ...
      val telTuple: (String, String) = orderInfo.consigneeTel.splitAt(3)

      val datetimeArr: Array[String] = orderInfo.createTime.split(" ")
      orderInfo.createDate=datetimeArr(0)
      orderInfo.createHour= datetimeArr(1).split(":")(0)

      orderInfo.consigneeTel = telTuple._1 + "********"
